...
void sobel_filter(unsigned char *data, int width, int height, unsigned char *newdata, int start, int end) {
    int rowSize = width * 3;
...
int main(int argc, char *argv[]) {
    ...
    if (!strcmp(argv[1], "grey")) {
        ...
        unsigned char *subDataWithBorders = (unsigned char *)malloc(extendedHeight * rowSize);
        ...
        if (rank != 0) {
            ...
            MPI_Send(subData, rowSize, MPI_UNSIGNED_CHAR, rank - 1, 0, MPI_COMM_WORLD);
            // Recibir última fila del proceso anterior
            MPI_Recv(subDataWithBorders, rowSize, MPI_UNSIGNED_CHAR, rank - 1, 1, MPI_COMM_WORLD, &status);
        } else {
        ...
        if (rank != size - 1) {
            ...
            MPI_Send(subData + (localHeight - 1) * rowSize, rowSize, MPI_UNSIGNED_CHAR, rank + 1, 1, MPI_COMM_WORLD);
            // Recibir primera fila del siguiente proceso
            MPI_Recv(subDataWithBorders + (localHeight + 1) * rowSize, rowSize, MPI_UNSIGNED_CHAR, rank + 1, 0, MPI_COMM_WORLD, &status);
        } else {
            // Replicar última fila
            memcpy(subDataWithBorders + (localHeight + 1) * rowSize, subData + (localHeight - 1) * rowSize, rowSize);
        }
        ...
        sobel_filter(subDataWithBorders, width, extendedHeight, subDataProcessed, start, end);
